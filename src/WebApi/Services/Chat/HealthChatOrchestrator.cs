    ...
    ILogger<HealthChatOrchestrator> logger)
{
        ...
        string? connectionId = null)
    {
        ...
        else
        {
            ...
            symptomChanges = await TrackEpisodeChangesAsync(userId, routedResponse.SymptomChanges, episodesBeforeDict);
            appointmentChanges = await TrackAppointmentChangesAsync(userId, appointmentsBefore);
            assessmentChanges = await TrackAssessmentChangesAsync(userId, conversation.Id, assessmentsBefore);
        }
